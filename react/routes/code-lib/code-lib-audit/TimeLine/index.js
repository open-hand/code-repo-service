...
const TimeLine = ({ formatMessage, activeProject, timeLineStore, optLogDs, activeTabKey }) => {
  ...
  useEffect(() => {
    if (activeTabKey === TabKeyEnum.OPTLOG) {
      if (activeProject.id && activeProject.id !== 'all') {
        optLogDs.setQueryParameter('projectId', activeProject.id);
      } else if (activeProject.id === 'all') {
        optLogDs.setQueryParameter('projectId', undefined);
      }
      loadData();
    }
  ...
  const timeLineProps = useMemo(() => ({ formatMessage, isMore, opEventTypeLookupData, loadData, optLogDs }), [isMore, opEventTypeLookupData, loadData, optLogDs]);
  return (
