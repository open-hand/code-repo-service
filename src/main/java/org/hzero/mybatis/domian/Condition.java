...
public class Condition implements IDynamicTableName {
    ...
    protected Criteria createCriteriaInternal() {
        Criteria criteria = new Criteria(propertyMap, exists, notNull, table.isMultiLanguage());
        return criteria;
    }
    ...
    protected abstract static class GeneratedCriteria {
        ...
        protected GeneratedCriteria(Map<String, EntityColumn> propertyMap, boolean exists, boolean notNull, boolean multiLanguage) {
            super();
            this.exists = exists;
            this.notNull = notNull;
        ...
        private String column(String property) {
            ...
            if (propertyMap.containsKey(property)) {
                return prefix + propertyMap.get(property).getColumn();
            } else if (exists) {
                throw new MapperException("当前实体类不包含名为" + property + "的属性!");
            } else {
                return null;
    ...
    public static class Criteria extends GeneratedCriteria {
        ...
        protected Criteria(Map<String, EntityColumn> propertyMap, boolean exists, boolean notNull, boolean multiLanguage) {
            super(propertyMap, exists, notNull, multiLanguage);
        }
    }
    ...
    public static class Builder {
        ...
        //动态表名
        private List<Criteria> conditionCriterias;
        //动态表名
        ...
        public Condition build() {
            ...
            for (Sqls.Criteria criteria : sqlsCriteria) {
                Criteria conditionCriteria = new Criteria(this.propertyMap, this.exists, this.notNull, this.table.isMultiLanguage());
        ...
        private void transformCriterion(Criteria conditionCriteria, String condition, String property, Object[] values, String andOr) {
            if (values.length == 0) {
                if ("and".equals(andOr)) {
            ...
            if (propertyMap.containsKey(property)) {
                ...
                if (EntityHelper.contain(entityClass)) {
                    ...
                    if (entityTable.isMultiLanguage()) {
                        if (!SqlHelper.TENANT_ID.equalsIgnoreCase(column)
                                && entityTable.getMultiLanguageColumns().stream().map(EntityColumn::getColumn).collect(Collectors.toList()).contains(column)) {
                            column = " t." + column;
                        } else {
                            column = " b." + column;
                        }
            ...
            if (EntityHelper.contain(entityClass)) {
                ...
                if (entityTable.isMultiLanguage()) {
                    ...
                    for (String selectColumn : selectColumns) {
                        if (!SqlHelper.TENANT_ID.equalsIgnoreCase(selectColumn) && multiLanguageColumns.contains(selectColumn)) {
                            selectColumnSet.add("t." + selectColumn);
                        } else if (SqlHelper.TENANT_ID.equalsIgnoreCase(selectColumn) || pkColumns.contains(selectColumn)) {
