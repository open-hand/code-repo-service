...
public class NonProjectMemberPermissionProcessor implements RolePermissionProcessor {
    ...
    public void repairProjectPermissionByRole(Member projectGlMember, RdmMember rdmMember, RdmMemberAuditRecord rdmMemberAuditRecord) {
        if (Objects.isNull(rdmMember)) {
            ...
            handProjectDBMemberExist(projectGlMember, rdmMember, rdmMemberAuditRecord);
        }
    }

    private void handProjectDBMemberExist(Member projectGlMember, RdmMember rdmMember, RdmMemberAuditRecord rdmMemberAuditRecord) {
        //如果gitlab组的权限为null,
        if (projectGlMember != null) {
            handProjectGlmemberExist(projectGlMember, rdmMember, rdmMemberAuditRecord);
        } else {
            // 如果gitlab组的权限为null,gitlab项目的权限也为null
            // 如果在choerodon是同步成功的 权限小于50，则按照choerodon来修复权限
            addProjectMember(rdmMember, rdmMemberAuditRecord);
        }
    }

    private void handProjectGlmemberExist(Member projectGlMember, RdmMember rdmMember, RdmMemberAuditRecord rdmMemberAuditRecord) {
        // gitlab项目的权限不为null
        ...
        updateProjectMember(projectGlMember, rdmMember, rdmMemberAuditRecord);
    }

    private void addProjectMember(RdmMember rdmMember, RdmMemberAuditRecord rdmMemberAuditRecord) {
        if (rdmMember.getSyncGitlabFlag() && rdmMember.getGlAccessLevel() < 50) {
            //在添加权限之前需要判断组的权限有没有
            RdmMember groupMember = queryRdmMember(rdmMemberAuditRecord);
            if (groupMember != null) {
                removeAndAddMember(rdmMemberAuditRecord, groupMember);
            } else {
    ...
    private void updateProjectMember(Member projectGlMember, RdmMember rdmMember, RdmMemberAuditRecord rdmMemberAuditRecord) {
    ...
    private void removeAndAddMember(RdmMemberAuditRecord rdmMemberAuditRecord, RdmMember groupMember) {
        gitlabGroupFixApi.removeMember(rdmMemberAuditRecord.getgGroupId(), rdmMemberAuditRecord.getGlUserId());
