...
To install the chart with the release name `code-repo-service`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install code-repo-service c7n/code-repo-service
```
...
| containerPort.actuatorPort | int | `7141` | server management port |
| containerPort.serverPort | int | `7140` | server port |
...
| extraEnv.GITLAB_PRIVATETOKEN | string | `"xxx"` |  |
...
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/hrds_code_repo?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&serverTimezone=Asia/Shanghai"` |  |
...
| image.repository | string | `"c7n/code-repo-service"` | service image repository |
...
| service.nodePort | object | `{"actuator":30171,"server":30170}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":7141,"server":7140}` | server Service port |
